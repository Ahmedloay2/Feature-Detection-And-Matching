...
// ─── Descriptor matchers ──────────────────────────────────────────────────────
// (Moved to processors/feature_matcher.cpp)
         ...
         sift1_kp, sift1_desc]()
        {
            ...
            try
            {
                ...
                for (int ri = 0; ri < static_cast<int>(rois.size()); ++ri)
                {
                    ...
                    if (selectedMode == 0)
                        roiMatches = cv_assign::matching::matchSSD(roiDesc, sift2_desc, ratio);
                    else
                        roiMatches = cv_assign::matching::matchNCC(roiDesc, sift2_desc, ratio);
                    totalMatchMs +=
